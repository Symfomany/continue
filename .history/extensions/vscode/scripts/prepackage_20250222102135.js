  ...
  async function installNodeModuleInTempDirAndCopyToCurrent(
    ...
    try {
      rimrafSync(`node_modules/${toCopy}`);
          ...
          console.warn(`[warn] Could not delete node_modules/${toCopy}:`, e);
        // Tentative de suppression forcée (ATTENTION : risqué)
        try {
          execCmdSync(`attrib -r node_modules/${toCopy} /s /d`); // Supprimer l'attribut "lecture seule"
          rimrafSync(`node_modules/${toCopy}`);
          console.log(`[info] Successfully force-deleted node_modules/${toCopy}`);
        } catch (e2) {
          console.error(`[error] Could not force-delete node_modules/${toCopy}:`, e2);
          // Ici, tu peux choisir d'abandonner l'installation ou de continuer malgré tout
          // throw e2; // Abandonner
          console.warn("[warn] Continuing despite deletion failure."); // Continuer
        }
      }
