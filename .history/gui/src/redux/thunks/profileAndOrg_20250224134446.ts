...
>("profiles/updatePersonna", async (personna: string | null, { dispatch }) => {
    // Ici, tu ferais ton appel API pour sauvegarder la personna (optionnel)
    // await api.savePersonna(personna); // Exemple

    dispatch(setPersonna(personna)); // Met à jour le state Redux
  })