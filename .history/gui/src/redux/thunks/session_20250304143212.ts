...
>(
  ...
  async ({ openNewSession, generateTitle }, { dispatch, extra, getState }) => {
    ...
    const sessionLite = {
      ...
      try {
      await dispatch(createSession({sessionLite}));
      posthog.capture("chat", sessionLite);

      console.log("Session saved:", getState().session);
      

      // Mettez à jour la session avec les nouvelles données
...
>("session/create", async ({ sessionLite }, thunkAPI) => {
  try {
    const state = thunkAPI.getState(); // Obtenez l'état global
    const session = state.session; // Accédez à la session depuis le state
    let sessionLiteUpgraded = {...sessionLite,...session, created: new Date().getTime()}
