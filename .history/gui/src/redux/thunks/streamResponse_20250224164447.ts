  ...
  async (
    ...
    await dispatch(
      streamThunkWrapper(async () => {
        ...
        const personna = state.config.personna; // Récupère personna depuis le state Redux
        ...
        const inputIndex = index ?? state.session.history.length; // Either given index or concat to end
         ...
         const prompt = `Tu es un assistant qui ${personna}.  Réponds à la question : ${editorState.content}`;
