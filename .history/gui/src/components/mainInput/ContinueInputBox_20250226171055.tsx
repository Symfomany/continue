...
function ContinueInputBox(props: ContinueInputBoxProps) {
  ...
  const toolbarOptions: ToolbarOptions = props.isEditMode
    ...
    useEffect(() => {
      ...
      console.log("laaaaaa !! ici et pk");
      
      if (selectedTemplate && selectedTemplate.content) {

        try {
            // Check if selectedTemplate.content is already a JSON object
            let parsedContent = typeof selectedTemplate.content === 'string'
                ? JSON.parse(selectedTemplate.content) // Attempt to parse string content
                : selectedTemplate.content; // Use directly if already an object
          console.log("parsedContent", parsedContent);
          // Validate parsedContent structure
          if (typeof parsedContent === 'object' && parsedContent !== null && parsedContent.type === 'doc') {
              setEditorState(parsedContent); // It's a JSON doc
          } else {
              // If not a JSON doc, handle as plain text
              console.error("Invalid template content format.");
              setEditorState(createTipTapDocument("Invalid template content format."));
          }
          ...
          console.error("Erreur de parsing JSON:", error);
            ...
            setEditorState(createTipTapDocument("")); // Reset to default state when no template selected
        }
    }, [selectedTemplate]);
