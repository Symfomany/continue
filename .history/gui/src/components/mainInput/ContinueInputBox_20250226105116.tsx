...
function ContinueInputBox(props: ContinueInputBoxProps) {
  ...
  const toolbarOptions: ToolbarOptions = props.isEditMode
    ...
    const [editorState, setEditorState] = useState(createTipTapDocument("Initial content\nWith a line break"));

    
    function createTipTapDocument(text: string): JSONContent {
      const paragraphs = text.split('\n');
      const content = paragraphs.filter(paragraph => paragraph.trim() !== "").map(paragraph => ({ // Filtrer les paragraphes vides
        type: 'paragraph',
        content: [{
          type: 'text',
          text: paragraph,
      ...
      return {
        type: 'doc',
        content: content.length > 0 ? content : [{ type: 'paragraph', content: [{ type: 'text', text: '' }] }], // Garantir qu'il y a au moins un paragraphe
      };
    }
 ...
 const handleTemplateSelect = useCallback(
      (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (selected) {
            console.log("selectedTemplate.content:", selected.content); // Ajout de la ligne de debug
          }
      },
      [messageTemplates],
    );
