...
const TokenCount = styled.div`
  ...
  font-size: 0.75rem;
  color: #888;
  background-color: rgba(255, 255, 255, 0.8); // Pour la lisibilité
  padding: 2px 4px;
  border-radius: 3px;
`;
...
function TipTapEditor(props: TipTapEditorProps) {
  ...
  // Fonction pour estimer le nombre de tokens (exemple simple)
  const estimateTokenCount = (text: any) => {
    // Remplacer ceci par une logique de tokenisation plus précise si nécessaire
    return text.split(/\s+/).length; // Simpliste : compte les mots comme tokens
  };
  ...
  const editor: Editor | null = useEditor({
    extensions: [
      ...
      CharacterCount.configure({
        // On surcharge la fonction de comptage
        textCounter: estimateTokenCount,
      }),
  ...
  function getPlaceholderText(
    ...
    historyLength: number,
        ...
        const welcomeMessages = [
          "Bienvenue ! Quel code souhaitez-vous générer aujourd'hui ?",
          "Salut ! Exprimez vos besoins en code.",
          "Hello ! Décrivez la fonction que vous imaginez.",
          "Bienvenue ! Indiquez le type de code dont vous avez besoin.",
          "Salut ! Quel problème de code puis-je résoudre pour vous ?",
          "Bienvenue ! Décrivez le script que vous voulez créer.",
          "Hello ! Prêt à coder ? Dites-moi quoi !",
          "Bienvenue ! Votre assistant code est à l'écoute."
        ];
    ...
    if (historyLength === 0) {
      // Si c'est la première interaction, on choisit un message de bienvenue aléatoire
      const randomIndex = Math.floor(Math.random() * welcomeMessages.length);
      return welcomeMessages[randomIndex] + " (Utilisez '@' pour le contexte)";
    } else {
       const messages = [
        "Continuez, je suis là pour vous aider à coder ! 🚀",
        "N'hésitez pas à me poser d'autres questions, même les plus pointues ! 😉",
        "On avance bien ! Que puis-je faire d'autre pour vous ? 😊",
        "Votre projet est passionnant ! Expliquez-moi la suite. 🤓",
        "Ne vous arrêtez pas en si bon chemin ! Je suis prêt à relever tous les défis. 💪",
        "On dirait que vous avez de grandes idées ! Comment puis-je vous aider à les concrétiser ? 💡",
        "Coding is fun! Alors, on continue ? 😄",
        "Votre question m'intéresse beaucoup ! Dites-m'en plus. 🤩",
        "Continuez la discussion, je suis votre assistant code préféré! 🤖"
      ];
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      editor.commands.setContent(props.editorState, true);
  ...
  useWebviewListener(
    ...
    async (data) => {
      ...
      // 1. Vérifier si rangeInFileWithContents existe
      if (!data.rangeInFileWithContents) {
        console.warn(
          "highlightedCode event received with empty rangeInFileWithContents. Ignoring.",
        );
        return;
      }

      // 2. Appeler rifWithContentsToContextItem
      const contextItem = rifWithContentsToContextItem(
        data.rangeInFileWithContents,
      );

      // 3. Valider contextItem et ses propriétés
      if (
        !contextItem ||
        !contextItem.id ||
        typeof contextItem.id !== "object" ||
        !("itemId" in contextItem.id)
      ) {
        console.error(
          "Invalid contextItem or missing itemId. Check rifWithContentsToContextItem implementation.",
          contextItem,
        );
        return; // Sortir si contextItem est invalide
      }
   
      // 4. Valider le contenu de l'éditeur et effectuer des opérations en toute sécurité
      const editorContent = editor.getJSON()?.content;
      if (!editorContent || !Array.isArray(editorContent)) {
        console.warn("Editor content is invalid or empty. Skipping code block insertion.");
        return;
      }
     
      // Trouver l'index pour l'insertion
      let index = 0;
      for (const el of editorContent) {
  ...
  return (
    ...
    >
      <div className="px-2.5 pb-1 pt-4">
        <StyledEditorContent
          className={`scroll-container overflow-y-scroll ${props.isMainInput ? "max-h-[80vh]" : ""}`}
          spellCheck={false}
          editor={editor}
          onClick={(event) => {
      ...
      {showDragOverMsg &&
        modelSupportsImages(
          ...
          <>
            <HoverDiv></HoverDiv>
            <HoverTextDiv>Déposer une image</HoverTextDiv>
          </>
        )}
