...
function countTokens(text: string, modelName: any) {
  try {
    ...
    // Free the encoder to prevent memory leaks (for long-running sessions)
    encoder.free();
...
function TipTapEditor(props: TipTapEditorProps) {
  ...
  const estimateTokenCount = (text: string) => {
    let tokenCount = 0;
    try {
        const modelName = "gpt-4-0125-preview";
        tokenCount = countTokens(text, modelName);
    } catch (error) {
        console.error("Error counting tokens:", error);
    }
  ...
  let tokenCount = 0 ; 
  if(editor){
    tokenCount =  editor.storage.characterCount.characters();
  }
