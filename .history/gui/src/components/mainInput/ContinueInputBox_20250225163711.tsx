  ...
  const toolbarOptions: ToolbarOptions = props.isEditMode
    ...
    const handleTemplateSelect = useCallback(
        (template: MessageTemplate) => {
            ...
            // Ici, tu dois déclencher la mise à jour du contenu de l'éditeur.
            // Puisque tu utilises TipTapEditor comme un composant contrôlé, tu dois passer
            // le contenu du template via la prop `editorState`.
            // Pour ça, il faut que TipTapEditor accepte une prop `initialContent` ou similaire.

            // Tu peux aussi utiliser la méthode `commands.setContent` de TipTap,
            // mais ça nécessite d'avoir accès à l'instance de l'éditeur.
            // Je vais te montrer comment faire les deux.
        },
        [],
    );
    ...
    <div className={`${props.hidden ? "hidden" : ""}`}>
      <div className={`relative flex flex-col px-2`}>
        ...
        >
          <TipTapEditor
           initialContent = {
                selectedTemplate ? selectedTemplate.content : undefined
            }
