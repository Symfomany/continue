...
function ConfigPage() {
  ...
  const handleEditTemplate = (template: any) => {
        console.log("template selected... ", JSON.parse(template.content));
...
useEffect(() => {
  ...
  const handleLogout = async () => {
    let sessionLite = {
        ...
        session:  typeof session === 'object' ? session.account : null, 
      }
      posthog.capture("logout", sessionLite);
    ...
    if (success) {
        ...
        let sessionLite = {
            action: "login",
            ide: isJetBrains() ? "Intellij" : "VSCode",
            session:  typeof success === 'object' ? success.account : null, 
          }
  ...
  function updatePersonna() {
    ...
    showToast('Personna mise à jour', { type: 'success' });
  }
  ...
  return (
    ...
    {session ? <div className="flex flex-col" style={{ paddingLeft: '10px' }}>
