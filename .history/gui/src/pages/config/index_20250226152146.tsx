...
function ConfigPage() {
  ...
  const handleEditTemplate = (template: any) => {
        ...
        setEditorContent(convertTextToTipTapContent(template.content)); // Mettre à jour editorContent
    };
  ...
  const handleEditorChange = useCallback((content: JSONContent) => {
    ...
    const textContent = content.content?.map(item => item.content?.map(textItem => textItem.text).join('')).join('\n') || '';
    setNewTemplateContent(textContent);
  }, []);

   useEffect(() => {
    setEditorContent(convertTextToTipTapContent(newTemplateContent));
  }, [newTemplateContent]);
  ...
  return (
                ...
                {hubEnabled ? (
                  ...
                  <>
                    ...
                    <small><i>Le personna sera utiliser en contexte des discussions</i></small>
                      ...
                      <Button className="bg-black-70 text-white"
    ...
    {session ? <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-2">
            {!editingTemplateId && messageTemplates.length ? <h4>Mes templates</h4> : <></>}
